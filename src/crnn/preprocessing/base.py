...
def calculate_horizon_and_window(
    transient_time: int,
    N: int,
    dt: float,
...
def calculate_normalization_parameters(
    ...
    tracker.track(
        ev.SaveNormalization("", input=normalization.input, output=normalization.output)
    )
...
def perform_data_splitting(
    ...
    ood_folder = os.path.join(
        dataset_dir, PREPARED_FOLDER_NAME, OUT_OF_DISTRIBUTION_FOLDER_NAME
    )
    id_folder = os.path.join(
        dataset_dir, PREPARED_FOLDER_NAME, IN_DISTRIBUTION_FOLDER_NAME
    )
    ...
    def split_into_subsequences(
        data: InputOutputList, seq_length: int
    ) -> InputOutputList:
        inputs, outputs = [], []
    ...
    if not ood_test_data.d:
        ...
        high_energy_train, train_data = separate_high_energy(
            train_data, train_energies, 0.1
        )
        high_energy_val, val_data = separate_high_energy(val_data, val_energies, 0.1)
        high_energy_test, test_data = separate_high_energy(
            test_data, test_energies, 0.1
        )
...
def preprocess(
    ...
    if data_was_prepared:
        tracker.track(
            ev.Log("", f"Dataset '{dataset_name}' was downloaded and prepared")
        )
    else:
    ...
    if not os.path.exists(prepare_folder_name):
        tracker.track(
            ev.Log(
                "",
                f"Prepared data folder does not exist at {prepare_folder_name}, performing data splitting.",
            )
        )
        ...
        tracker.track(
            ev.Log(
                "",
                f"Prepared data folder already exists at {prepare_folder_name}, skipping data splitting.",
            )
        )
