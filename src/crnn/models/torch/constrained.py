    ...
    def sdp_constraints(self) -> List[Callable]:
        def stability_lmi() -> torch.Tensor:
            ...
            M11 = trans.torch_bmat(
                [
                    [-self.X, torch.zeros((self.nx, self.nd)), self.C2_tilde.T], 
                    [torch.zeros((self.nd, self.nx)), -self.ga2 * torch.eye(self.nd), self.D21_tilde.T],
            ...
            M22 = trans.torch_bmat([
                [-self.X, torch.zeros((self.nx, self.ne))],
...
class ConstrainedLtiRnnGeneralSectorConditions(base_torch.L2StableConstrainedModule):
    ...
    def sdp_constraints(self) -> List[Callable]:
        def stability_lmi() -> torch.Tensor:
            ...
            M11 = trans.torch_bmat(
                [
                    [-self.X, torch.zeros((self.nx, self.nd)), (self.C2_tilde - self.H).T], 
                    [torch.zeros((self.nd, self.nx)), -self.ga2 * torch.eye(self.nd), self.D21_tilde.T],
    ...
    def project_parameters(self) -> str:
        ...
        return f'Projected parameters with cvxpy solution, ga2: {ga2.value}, problem status: {problem.status}'